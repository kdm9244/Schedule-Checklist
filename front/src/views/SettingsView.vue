<template>
  <div class="settings-page">
    <header class="settings-header">
      <div><span>SETTINGS</span><h1>設定</h1><p>チェックリストの自動作成と、画面に表示するGoogleカレンダーを管理します。</p></div>
    </header>
    ...
    <main v-if="activeTab==='templates'" class="settings-layout">
      <section class="template-form">
        ...
        <p class="form-note">開始日より前には生成されません。対象日の最初の表示時に一度だけ生成され、過去に作成済みの項目はそのまま残ります。</p>
    ...
    <main v-else class="calendar-settings-card">
      <div class="calendar-settings-heading">
        <div><span>DISPLAY CALENDARS</span><h2>表示する予定</h2><p>オフにしたカレンダーは「今日」と月間カレンダーの両方から非表示になります。Google側の予定は変更されません。</p></div>
...
@media(max-width:950px){.settings-page{height:auto}.settings-layout{grid-template-columns:1fr;overflow:visible}.template-form{border-right:0;border-bottom:1px solid #e2e7ef}.calendar-groups{grid-template-columns:1fr}.calendar-settings-card{overflow:visible}}
